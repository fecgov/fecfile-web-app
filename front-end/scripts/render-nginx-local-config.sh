...
if [[ "$WATCH" == "1" ]]; then
  ...
  browser_sync_proxy_location='location /browser-sync/ { resolver 127.0.0.11 valid=30s; set $bs_upstream http://browser-sync:3000; proxy_pass $bs_upstream; proxy_http_version 1.1; proxy_set_header Host $host; proxy_set_header Upgrade $http_upgrade; proxy_set_header Connection "upgrade"; proxy_intercept_errors on; error_page 502 503 504 =204 /browser-sync-empty; }'
