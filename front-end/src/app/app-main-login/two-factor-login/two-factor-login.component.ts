...
export class TwoFactorLoginComponent {
  ...
  constructor(private router: Router, private _fb: FormBuilder, private _messageService: MessageService) {
  ...
  submit() {
    ...
    if (this.twoFactInfo.valid) {
      this._messageService.sendMessage({ action: 'sendSecurityCode', selectedOption: option, entryPoint: 'login' });
