...
export class F1mQualificationComponent implements OnInit, OnDestroy {
  ...
  public getNextCandidateNumber(): number {
    ...
    if (this.qualificationData.candidates && this.qualificationData.candidates.length > 0) {
      this.qualificationData.candidates.sort((a: any, b: any) => (a.candidate_number > b.candidate_number ? 1 : -1));
      this.qualificationData.candidates.forEach((candidate: any) => {
  ...
  private setRequirementsMetDate() {
    this._f1mService
      .getDates('get_cmte_met_req_date', this.reportId, this.formPart2.value.fifty_first_contributor_date)
      .subscribe((res) => {
        if (this.formPart2) {
