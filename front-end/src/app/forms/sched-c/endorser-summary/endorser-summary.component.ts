...
export class EndorserSummaryComponent implements OnInit, OnDestroy {
  ...
  // Local Storage Keys
  private readonly endorserSortableColumnsLSK = 'endorser.ctn.sortableColumn';
  private readonly recycleSortableColumnsLSK = 'endorser.recycle.sortableColumn';
  private readonly endorserCurrentSortedColLSK = 'endorser.ctn.currentSortedColumn';
  private readonly recycleCurrentSortedColLSK = 'endorser.recycle.currentSortedColumn';
  private readonly endorserPageLSK = 'endorser.ctn.page';
  private readonly recyclePageLSK = 'endorser.recycle.page';
  private readonly filtersLSK = 'endorser.filters';
  ...
  public getendorserPage(page: number): void {
    ...
    this._LoanService
      .getEndorsers(this.transactionDetail.endorser.back_ref_transaction_id)
  ...
  public determineItemRange(): string {
    ...
    if (this.config.currentPage > 0 && this.config.itemsPerPage > 0 && this.endorserModel.length > 0) {
  ...
  private applyColumnsSelected() {
    ...
    if (localStorage.getItem(key) != null) {
      ...
      for (const col of ctnCols) {
        let vis = this._tableService.getColumnByName(col.colName, this.sortableColumns);
        if (vis) {
    ...
    switch (this.tableType) {
      case this.endorserView:
        this.setCacheValuesforView(
          this.endorserSortableColumnsLSK,
          this.endorserCurrentSortedColLSK,
  ...
  public trashEndorser(endorser: any): void {
    this._dialogService
      .confirm(
        'You are about to delete this transaction ' + endorser.transaction_id + '.',
        ConfirmModalComponent,
        'Caution!'
      )
      .then((res: any) => {
