...
export class CashOnHandComponent implements OnInit {
  ...
  showWarning() {
    const warningMessage =
      'Editing the First Time Cash on Hand may impact reports that have already been filed or saved. If you edit the Cash on Hand it will automatically amend any filed reports.';
    if (this.editMode && !this.editWarningAlreadyShown) {
      this._dialogService
        .confirm(warningMessage, ConfirmModalComponent, 'Warning!', false, ModalHeaderClassEnum.warningHeader)
        .then((res) => {
          //do nothing.
        });
