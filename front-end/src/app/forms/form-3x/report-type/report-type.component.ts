...
import { ReportFilterModel } from 'app/reports/model/report-filter.model';
...
export class ReportTypeComponent implements OnInit, OnDestroy {
  ...
  public fromDateSelected: string = '';
  public toDateSelected: string = '';
  private _reportTypeDescripton: string = '';
  private _selectedElectionState: string = '';
  private _selectedElectionDate: string = '';
  private _fromDateUserModified: string = '';
  private _toDateUserModified: string = '';
  private onDestroy$ = new Subject();
  invalidDatesServerValidation!: boolean;

  public selectedState: string = '';
  public selectedElectionDate: string = '';
  ...
  constructor(
    ...
    this._messageService
      .getUpdateReportTypeMessageToReportType()
      ...
      .subscribe((res) => {
        if (res && res.action === 'coverageDatesUpdated') {
          ...
          if (
            ...
            !this.selectedReportInfo.disableCoverageDates &&
            (!res.currentData.fromDate || !res.currentData.toDate)
          ) {
  ...
  private updateReportTypeFromMessage(message: any) {
    ...
    if (currentReportDeepCopy && currentReportDeepCopy.length > 0) {
      ...
      this.updateTypeSelectedForEdit(
        currentReportDeepCopy.report_type,
        currentReportDeepCopy.regular_special_report_ind
      );
    }
  }
      ...
      if (res) {
        ...
        if (Array.isArray(res) && !res[0].hasOwnProperty('create_date')) {
          ...
          window.localStorage.setItem(
            `form_${this._formType}_report_type`,
            JSON.stringify(this._formReportTypeDetails)
          );
          return 0;
        }
        this.status.emit({
  ...
  private handleForm(editMode: boolean = false, formType: string) {
    ...
    if (this.semiAnnualStartDate) {
      this._formReportTypeDetails.semi_annual_start_date = this._datePipe.transform(
        this.semiAnnualStartDate,
        'MM/dd/yyyy'
      );
    }
  ...
  private showOrphanTransactionsExistWarningMessage(data: any) {
    this._dialogService
      .confirm(
        'By changing this report, there now exist one or more transactions that are not tied to any report. In order to show those transactions, please ensure that there is a report created corresponding to those coverage dates',
        ConfirmModalComponent,
        'Warning',
        true,
        ModalHeaderClassEnum.warningHeader,
  ...
  private showReportExistsWarningMessage(res: any, reportId: number) {
    ...
    let reportStatus = '';
    this._reportService
      .getReports('current', 1, 1, '', true, new ReportFilterModel(), reportId)
      .subscribe((reportRes) => {
        if (reportRes) {
