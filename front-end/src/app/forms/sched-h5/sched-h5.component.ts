...
export class SchedH5Component extends AbstractSchedule implements OnInit, OnDestroy, OnChanges {
  ...
  public override loaded = true; //false;
  ...
  public saveAddMore(): void {
    ...
    const total_amount_transferred =
      this.convertFormattedAmountToDecimal(this.schedH5.get('total_amount_transferred')?.value) +
  ...
  public setH5Sum(page: number) {
    ...
    this.h5Subscription = this._schedH5Service
  ...
  public editH5(item: any) {
    ...
    this.schedH5.patchValue(
      { transferred_amount: this._decPipe.transform(item.transfer_amount, '.2-2') },
      { onlySelf: true }
    );
    this.schedH5.patchValue(
  ...
  public editH5Sub(item: any) {
    ...
    this.schedH5.patchValue(
      {
        transferred_amount: this._decPipe.transform(
          this.convertFormattedAmountToDecimal(item.transferred_amount),
          '.2-2'
        ),
      },
      { onlySelf: true }
    ...
    this.h5Entries = this.h5Entries.filter((obj: any) => obj !== item);
    this.h5Ratios.child = this.h5Ratios.child.filter((obj: any) => obj !== item);

    let sum = 0;
    this.h5Entries.forEach((obj: any) => {
  ...
  public trashSubTransaction(trx: any): void {
    this._dlService
      ...
      .then((res: any) => {
        if (res === 'okay') {
          this.h5Entries = this.h5Entries.filter((obj: any) => obj !== trx);
          this.h5Ratios.child = this.h5Ratios.child.filter((obj: any) => obj !== trx);
          this.config.totalItems = this.h5Entries.length;
  ...
  private _noH1H2Popup(category: string) {
    ...
    this.getH1H2ExistSubscription = this._schedH5Service
