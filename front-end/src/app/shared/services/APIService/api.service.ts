...
export class ApiService implements HttpInterceptor {
  ...
  public getCommiteeDetails(): Observable<any> {
    ...
    return this._http.get(`${this._appConfigService.getConfig().apiUrl}/core/get_committee_details`, {
      headers: httpOptions,
    });
  }
  ...
  getCommiteeDetailsForUnregisteredUsers() {
    ...
    return this._http.get(`${this._appConfigService.getConfig().apiUrl}/user/register/get_committee_details`, {
      headers: httpOptions,
    });
  }
    ...
    if (request.params.has('handleResponseError')) {
      return next.handle(request).pipe(
        catchError((error: HttpErrorResponse) => {
          ...
          this._dialogService.confirm(reason, ConfirmModalComponent, 'Exception', false).then((response) => {
            if (response === 'okay') {
              this._dialogService.checkIfModalOpen();
            } else if (
              response === 'cancel' ||
