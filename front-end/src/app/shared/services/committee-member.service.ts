...
export class CommitteeMemberService implements TableListService<CommitteeMember> {
  ...
  public async getTableData(pageNumber = 1, ordering = '', params: QueryParams = {}): Promise<ListRestResponse> {
    ...
    const response = await this.apiService.get<ListRestResponse>(`/committee-members/${parameter_string}`, params);
    response.results = response.results.map((item) => CommitteeMember.fromJSON(item));
    return response;
  }

  public async getMembers(): Promise<CommitteeMember[]> {
    const response = await this.apiService.get<Array<CommitteeMember>>('/committee-members/');
    const members = response.map((item) => CommitteeMember.fromJSON(item));
    return members;
  }

  public async addMember(email: string, role: typeof CommitteeMemberRoles): Promise<CommitteeMember> {
    const response = await this.apiService.post('/committee-members/add-member/', { email: email, role: role });
