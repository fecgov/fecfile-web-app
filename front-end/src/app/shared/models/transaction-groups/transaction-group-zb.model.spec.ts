...
describe('TransactionGroupZB', () => {
  ...
  it('#test group methods to ensure they return the right values', () => {
    ...
    expect(component.getParentFooter()).toBe(
      'The information in this loan will automatically populate a related transaction. Review the associated loan and enter a purpose of receipt or note/memo text; or click "Save both transactions" to record these transactions.'
    );
    expect(component.getGroupDescription()).toBe(
      'This loan type automatically creates an associated transaction. Saving a loan received from individual will automatically create an associated receipt.'
    );
    expect(component.getParentAccordionTitle()).toBe('ENTER DATA');
    expect(component.getParentAccordionSubTitle()).toBe('Add contact and loan information');
    expect(component.getChildAccordionTitle()).toBe('AUTO-POPULATED');
    expect(component.getChildAccordionSubTitle()).toBe('Review receipt information');
