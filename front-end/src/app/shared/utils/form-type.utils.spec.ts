...
import { FormType, FormTypes, getFormTypes } from './form-type.utils';
...
describe('FormTypeUtils', () => {
  ...
  describe('getFormTypes', () => {
    ...
    it('should have F3X', async () => {
      const F3X = getFormTypes(false).get(FormTypes.F3X);
    ...
    it('should have F3 if environment has showForm3', async () => {
      const F3 = getFormTypes(true).get(FormTypes.F3);
    ...
    it('should not have F3 if environment does not have showForm3', async () => {
      const F3 = getFormTypes(false).get(FormTypes.F3);
      expect(F3).toBeFalsy();
    });
  });
});
