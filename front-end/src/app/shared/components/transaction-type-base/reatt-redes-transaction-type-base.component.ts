  ...
  implements OnInit, OnDestroy
{
  ...
  override ngOnInit(): void {
    ...
    if (
      (transaction as SchATransaction | SchBTransaction).reattribution_redesignation_tag ===
    ...
    ReattRedesUtils.overlayForms(
      this.form,
      transaction as SchATransaction | SchBTransaction,
      this.childForm,
  ...
  private async initializePullForward() {
    ...
    this.reattributedData.form = this.fb.group(
      SchemaUtils.getFormGroupFieldsNoBlur(this.reattributedData.formProperties, reatRedes.transactionType.schema),
      {
        updateOn: 'blur',
      },
    );
