...
export const individualSchema = schema<IndividualContactData>((schemaPath) => {
  const schemaFieldMap = SchemaUtils.generatePathMapFromForm(defaultIndividualData);
  SchemaUtils.schemaFormValidatorBuilder(IndividualSchema, schemaPath, schemaFieldMap);
