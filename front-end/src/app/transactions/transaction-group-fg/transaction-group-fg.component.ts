...
export class TransactionGroupFgComponent extends TransactionTypeX2BaseComponent implements OnInit, OnDestroy {
  ...
  override ngOnInit(): void {
    ...
    // Group F contribution purpose description updates with Group G contributor name updates.
    this.childForm
    ...
    this.childForm
      ...
      ?.valueChanges.pipe(takeUntil(this.destroy$))
      .subscribe((value) => {
        const pacEarmarkMemo: SchATransaction = this.transactionType?.childTransactionType?.transaction as SchATransaction;
        if (pacEarmarkMemo) {
          pacEarmarkMemo.contributor_first_name = value;
        }
        updateContributionPurposeDescription();
      });
    this.childForm
      ...
      .subscribe((value) => {
        const pacEarmarkMemo: SchATransaction = this.transactionType?.childTransactionType?.transaction as SchATransaction;
        if (pacEarmarkMemo) {
          pacEarmarkMemo.contributor_last_name = value;
        }
        updateContributionPurposeDescription();
      });

    // Group G amount must match Group F contribution amount
    this.form
