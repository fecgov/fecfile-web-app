...
export class ResetSelectorComponent implements OnInit {
  ...
  submit() {
    ...
    if (this.frmUserInfo.valid) {
      ...
      this.twoFactorHelper.requestCode(option, callFrom).subscribe((response: any) => {
        if (response && response['is_allowed'] === true) {
          this._messageService.sendMessage({ action: 'sendSecurityCode', selectedOption: option, entryPoint: 'reset' });
