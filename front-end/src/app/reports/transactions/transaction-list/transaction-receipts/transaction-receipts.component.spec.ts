...
import { getTestIndividualReceipt, testActiveReport, testMockStore } from 'app/shared/utils/unit-test.utils';
...
import { ScheduleATransactionTypes, UploadSubmission } from 'app/shared/models';
...
describe('TransactionReceiptsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    testItemService = TestBed.inject(TransactionListService);
  ...
  describe('report is editable', () => {
    beforeEach(() => {
      const report = testActiveReport();
      report.upload_submission = undefined;
      fixture = TestBed.createComponent(TransactionReceiptsComponent, {
        bindings: [inputBinding('report', signal(report))],
      });
    ...
    it('should show the correct row actions', () => {
      expect(component.reportIsEditable()).toBeTruthy();
      ...
      expect(deleteAction?.isAvailable(transaction)).toBe(true);
      expect(editAction?.isAvailable(transaction)).toBe(true);
      expect(aggregateAction?.isAvailable(transaction)).toBe(true);

      expect(
        unaggregateAction?.isAvailable({ ...transaction, force_unaggregated: false } as TransactionListRecord),
      ).toBe(true);
      expect(itemizeAction?.isAvailable({ ...transaction, itemized: false } as TransactionListRecord)).toBe(true);
      expect(unitemizeAction?.isAvailable({ ...transaction, itemized: true } as TransactionListRecord)).toBe(true);
      ...
      expect(reattributeAction?.isAvailable({ ...transaction, itemized: false } as TransactionListRecord)).toEqual(
        true,
      );
    ...
    it('test canClone', () => {
      ...
      const isCloneableSpy = vi.spyOn(component.transactionService, 'isCloneable').mockReturnValue(true);
      expect(component.reportIsEditable()).toBeTruthy();
  ...
  describe('report is not editable', () => {
    beforeEach(() => {
      const report = testActiveReport();
      report.upload_submission = UploadSubmission.fromJSON({ fec_status: 'ACCEPTED', fecfile_task_state: 'SUCCESS' });
      fixture = TestBed.createComponent(TransactionReceiptsComponent, {
        bindings: [inputBinding('report', signal(report))],
      });
      component = fixture.componentInstance;
    ...
    it('should show the correct row actions', () => {
      ...
      expect(component.reportIsEditable()).toBeFalsy();
    ...
    it('test canClone', () => {
      const transaction = getTestIndividualReceipt() as unknown as TransactionListRecord;
      vi.spyOn(component.transactionService, 'isCloneable').mockReturnValue(true);
      expect(component.reportIsEditable()).toBeFalsy();
