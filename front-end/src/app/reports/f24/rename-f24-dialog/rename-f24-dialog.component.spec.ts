...
describe('RenameF24DialogComponent', () => {
  ...
  it('should not call update with no name', async () => {
    vi.spyOn(form24Service, 'update').mockResolvedValue(new Form24());
    vi.spyOn(form24Service, 'getAllReports').mockResolvedValue([]);
  ...
  it('should not call update with duplicate name', async () => {
    ...
    vi.spyOn(form24Service, 'getAllReports').mockResolvedValue([testF24Report]);
