...
describe('QA Test Script #133 (Sprint 8)', () => {
  it('', () => {
    ...
    //Step 3: Set the "Has your address changed" radio-button to "NO"
    ...
    cy.get('.p-menubar').find('.p-menuitem-link').contains('Reports').click();
    cy.shortWait();
    cy.createReport(report_2);

    //Step 7: Reopen the report
    ...
    //Step 9: Set the "Has your address changed" radio-button to "YES"
    ...
    //Step 10: Verify that the "Address" fields are displayed
    ...
    //Step 12: Check that the address changed
    cy.then(() => {
      ...
      cy.request({
        ...
        if (reportStreet_2 != '') {
          cy.expect(report.street_2).to.eql(reportStreet_2);
        } else {
          cy.expect(report.street_2).to.eql(null);
        }
