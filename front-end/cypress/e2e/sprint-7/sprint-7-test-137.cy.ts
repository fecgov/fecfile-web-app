...
describe('QA Test Script #137 (Sprint 7)', () => {
  ...
  it('', () => {
    ...
    //Step 3: Check for the existence of the FROM and TO fields
    cy.get('label[for="coverage_from_date"]').should('exist');
    cy.get('label[for="coverage_through_date"]').should('exist');

    //Step 4: Enter any date into the FROM and TO fields
    ...
    //Step 5: Checks that the dates entered are displayed in their fields
