...
import {
  committeeFormData,
  ContactFormData,
  defaultFormData as defaultContactFormData,
...
describe('Receipt Transactions', () => {
  ...
  it('Create an Individual Receipt transaction using the contact lookup', () => {
    cy.wrap(F3XSetup({ individual: true })).then((result: any) => {
      ...
      cy.get('[id="searchBox"]').type(individual['last_name'].slice(0, 3));
      cy.contains(individual['last_name']).should('exist');
      cy.contains(individual['last_name']).click({ force: true });
